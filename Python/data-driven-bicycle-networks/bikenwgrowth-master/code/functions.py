def normalise_edge_dict(dictionary):
    ...
    min_max_scaler = preprocessing.MinMaxScaler(feature_range=(0.04, 1.0)) #custom range
...
def normalise_list(listt):
    x = pd.DataFrame(listt) # convert dict to dataframe and transpose
    min_max_scaler = preprocessing.MinMaxScaler(feature_range=(0.0, 1.0)) #custom range
    x_scaled = min_max_scaler.fit_transform(x)
    x_scaled = [item for sublist in x_scaled for item in sublist] #flatten
...
def normalise_edge_dict2(dictionary):
    x = pd.DataFrame.from_dict([dictionary]).T # convert dict to dataframe and transpose
    min_max_scaler = preprocessing.MinMaxScaler(feature_range=(0.04, 1.0)) #custom range
    x_scaled = min_max_scaler.fit_transform(x)
    x_scaled = [item for sublist in x_scaled for item in sublist] #flatten
...
def normalise_graph_attr(G, attr):
    e_dict =  nx.get_edge_attributes(G, attr)
...
def osm_to_ig(node, edge):
    ...
    for i in range(len(edge)):
        ...
        edge_info["pop_den"].append(round(edge['pop_den'][i], 10))
    ...
    for i in range(len(edge)):
        ...
        G.es[i]["pop_den"] = edge_info["pop_den"][i]
...
def calculate_metrics(G, GT_abstract, G_big, nnids, calcmetrics = {#"length":0,
    ...
    if G.ecount() > 0 and GT_abstract.ecount() > 0:
        ...
        # POP DEN
        #if verbose and ("pop_den" in calcmetrics): print("Calculating pop_den...")
        #if "pop_den" in calcmetrics:
         #   output["pop_den"] = sum([e['pop_den'] for e in G.es])
            
          # POP DEN x length
        if verbose and ("pop_den_length" in calcmetrics): print("Calculating pop_den_length...")
        if "pop_den_length" in calcmetrics:
            output["pop_den_length"] = sum([e['pop_den']*e['length'] for e in G.es])
        
        # EDGE COUNT
        #if verbose and ("edge_count" in calcmetrics): print("Calculating edge_count...")
        #if "edge_count" in calcmetrics:
         #   output["edge_count"] = len(G.es)
...
def intersect_igraphs(G1, G2):
   ...
   #MÅSKE FJERN IF STATEMENT#
    #if 'pop_den' in edge_attribute_name_list:
    #    edge_attribute_name_list.remove('pop_den')
    #
